...
TEST_F(ScenarioProtocolTest, AcceptsAuthorityBattleStackRemovalStep)
{
	const std::string scenario = replaceToken(VALID_SCENARIO,
		"{\"op\": \"wait_frames\", \"frames\": 2}",
		"{\"op\": \"authority_remove_battle_stack\", \"stack_id\": 7}");
	const auto result = parseScenarioText(scenario);

	ASSERT_TRUE(result.ok());
	EXPECT_EQ(result.scenario->steps.at(1).kind, ScenarioStep::Kind::AUTHORITY_REMOVE_BATTLE_STACK);
	EXPECT_EQ(result.scenario->steps.at(1).stackId, 7);
}

TEST_F(ScenarioProtocolTest, AcceptsAuthorityActiveBattleStackStep)
{
	const std::string scenario = replaceToken(VALID_SCENARIO,
		"{\"op\": \"wait_frames\", \"frames\": 2}",
		"{\"op\": \"authority_set_active_battle_stack\", \"stack_id\": 7}");
	const auto result = parseScenarioText(scenario);

	ASSERT_TRUE(result.ok());
	EXPECT_EQ(result.scenario->steps.at(1).kind, ScenarioStep::Kind::AUTHORITY_SET_ACTIVE_BATTLE_STACK);
	EXPECT_EQ(result.scenario->steps.at(1).stackId, 7);
}
...
TEST_F(ScenarioProtocolTest, AcceptsManualHoldStep)
{
	...
	ASSERT_TRUE(result.ok());
	EXPECT_EQ(result.scenario->steps.at(1).kind, ScenarioStep::Kind::MANUAL_HOLD);
}
...
TEST_F(ScenarioProtocolTest, AcceptsBoundedTextInput)
{
	const std::string scenario = replaceToken(VALID_SCENARIO,
		"{\"op\": \"wait_frames\", \"frames\": 2}",
		"{\"op\": \"text_input\", \"text\": \"no-such-spell\"}");
	const auto result = parseScenarioText(scenario);

	ASSERT_TRUE(result.ok());
	ASSERT_EQ(result.scenario->steps.at(1).kind, ScenarioStep::Kind::TEXT_INPUT);
	EXPECT_EQ(result.scenario->steps.at(1).text, "no-such-spell");
}

TEST_F(ScenarioProtocolTest, RejectsTextInputBeyondSdlPayload)
{
	const std::string scenario = replaceToken(VALID_SCENARIO,
		"{\"op\": \"wait_frames\", \"frames\": 2}",
		"{\"op\": \"text_input\", \"text\": \"12345678901234567890123456789012\"}");
