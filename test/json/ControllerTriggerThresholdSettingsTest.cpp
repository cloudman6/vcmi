...
constexpr char KEY_MISSPELLED[] = "controllerTriggerTreshold";
...
TEST(SettingsSchemaTest, misspelledTriggerThresholdKeyIsNotDeclared)
{
	JsonNode config = maximizedSettings();

	EXPECT_EQ(config["input"].Struct().count(KEY_MISSPELLED), 0);
}
