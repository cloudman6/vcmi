...
bool ControllerE2EExecutor::applyJoystickBindingFixture(const std::string & fixtureField,
	const std::string & configGroup, std::string & error)
{
	const JsonNode & overrides = scenario.fixture[fixtureField];
	if(overrides.getType() == JsonNode::JsonType::DATA_NULL)
		return true;
	if(!overrides.isStruct())
	{
		error = "fixture." + fixtureField + " must be an object";
		return false;
	}

	for(const auto & [action, bindings] : overrides.Struct())
	{
		if(keyBindingsConfig[configGroup].Struct().count(action) == 0)
		{
			error = "fixture." + fixtureField + " references unknown action '" + action + "'";
			return false;
		}
		if(!bindings.isString() && !bindings.isVector())
		{
			error = "fixture." + fixtureField + " entry '" + action + "' must be a string or array";
			return false;
		}
		...
		{
			error = "fixture." + fixtureField + " entry '" + action + "' contains a non-string binding";
			return false;
		}
	}

	{
		Settings bindingSettings = keyBindingsConfig.write[configGroup];
		for(const auto & [action, bindings] : overrides.Struct())
			bindingSettings[action] = bindings;
	}
	return true;
}
...
void ControllerE2EExecutor::registerBuiltinProbes()
{
	...
	registry.registerProbe("runtime", []()
	{
		...
		if(ENGINE)
		{
			...
			snapshot["left_trigger_pressed"].Bool() = ENGINE->input().isControllerAxisPressedForE2E(
				SDL_CONTROLLER_AXIS_TRIGGERLEFT);
			snapshot["left_trigger_owned_by_active_controller"].Bool()
				= ENGINE->input().isControllerAxisOwnedByActiveControllerForE2E(SDL_CONTROLLER_AXIS_TRIGGERLEFT);
	...
	registry.registerProbe("navigation", []()
	{
		...
		if(ENGINE)
		{
			...
			const auto infoWindow = ENGINE->windows().topWindow<CInfoWindow>();
			snapshot["top_window_is_info"].Bool() = infoWindow != nullptr;
...
void ControllerE2EExecutor::activate()
{
	...
	std::string error;
	if(!applyJoystickBindingFixture("joystickAxisBindings", "joystickAxes", error)
		|| !applyJoystickBindingFixture("joystickButtonBindings", "joystickButtons", error))
	{
...
ControllerE2EExecutor::applyPrePollStep(const ScenarioStep & step, std::string & error)
{
	...
	case ScenarioStep::Kind::TEXT_INPUT:
	{
		...
		event.text.type = event.type;
		std::copy(step.text.begin(), step.text.end(), event.text.text);
		event.text.text[step.text.size()] = '\0';
		if(SDL_PushEvent(&event) != 1)
		{
			error = "SDL_PushEvent failed: " + std::string(SDL_GetError());
			return StepApplyResult::FAILED;
	...
	{
		if(!GAME || !GAME->interface() || !CPlayerInterface::battleInt || !GAME->server().client)
		{
			error = "authority battle stack removal requires an active local battle client";
			return StepApplyResult::FAILED;
		}

		BattleUnitsChanged pack;
		pack.battleID = CPlayerInterface::battleInt->getBattleID();
		pack.changedStacks.emplace_back(
			static_cast<uint32_t>(step.stackId), UnitChanges::EOperation::REMOVE);
	...
	case ScenarioStep::Kind::AUTHORITY_SET_ACTIVE_BATTLE_STACK:
	{
		if(!GAME || !GAME->interface() || !CPlayerInterface::battleInt || !GAME->server().client)
		{
			error = "authority active stack change requires an active local battle client";
			return StepApplyResult::FAILED;
		}

		if(pendingAuthorityPack.valid())
		{
			error = "another authority pack is still being applied";
			return StepApplyResult::FAILED;
		}

		const BattleID battleID = CPlayerInterface::battleInt->getBattleID();
		const uint32_t stackId = static_cast<uint32_t>(step.stackId);
		pendingAuthorityPack = std::async(std::launch::async, [battleID, stackId]()
		{
			std::unique_lock interfaceLock(ENGINE->interfaceMutex);
...
bool ControllerE2EExecutor::collectPendingAuthorityPack()
{
	if(!pendingAuthorityPack.valid()
		|| pendingAuthorityPack.wait_for(std::chrono::seconds(0)) != std::future_status::ready)
		return true;

	try
	{
		pendingAuthorityPack.get();
	}
	catch(const std::exception & e)
	{
		fail(E2E_DRIVER_ERROR, "authority pack failed: " + std::string(e.what()));
		return false;
	}
	return true;
}
...
bool ControllerE2EExecutor::evaluateCondition(bool & satisfied, std::string & error)
{
	...
	if(!readError.empty())
	{
		if(readError.find(" has no field '") != std::string::npos)
		{
			satisfied = false;
			return true;
		}
...
void ControllerE2EExecutor::onBeforePoll()
{
	...
	if(!collectPendingAuthorityPack())
		return;
...
void ControllerE2EExecutor::onAfterPresent()
{
	...
	if(!manualHoldActive
		&& std::chrono::duration_cast<std::chrono::milliseconds>(now - startTime).count() > scenario.timeoutMs)
	{
		fail(
			E2E_TIMEOUT,
...
int ControllerE2EExecutor::finalize()
{
	if(!finished)
	{
		...
		{
			exitCode = E2E_PASS;
			resultMessages.push_back("manual journey ended by normal application exit");
		}
		else
		{
