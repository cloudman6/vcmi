...
const std::map<ScenarioStep::Kind, std::set<std::string>> ALLOWED_FIELDS = {
	...
	{ScenarioStep::Kind::RECONNECT, {"op", "device"}},
	...
	{ScenarioStep::Kind::CLICK_MOUSE, {"op", "button", "x", "y"}},
...
bool requireStringField(const JsonNode & step, const std::string & field, std::string & target, const std::string & where, std::vector<std::string> & errors)
{
...
bool requireIntField(const JsonNode & step, const std::string & field, int & target, int minValue, int maxValue, const std::string & where, std::vector<std::string> & errors)
{
	const auto & value = step[field];
	if(value.getType() != JsonNode::JsonType::DATA_INTEGER || value.Integer() < minValue || value.Integer() > maxValue)
	{
		errors.push_back(where + ": field '" + field + "' must be an integer in [" + std::to_string(minValue) + ", " + std::to_string(maxValue) + "]");
		return false;
	}
...
bool requireFloatField(const JsonNode & step, const std::string & field, double & target, double minValue, double maxValue, const std::string & where, std::vector<std::string> & errors)
{
	const auto & value = step[field];
	if(!value.isNumber())
	{
	...
	const double number = value.getType() == JsonNode::JsonType::DATA_INTEGER ? static_cast<double>(value.Integer()) : value.Float();
	if(number < minValue || number > maxValue)
	{
		errors.push_back(where + ": field '" + field + "' must be in [" + std::to_string(minValue) + ", " + std::to_string(maxValue) + "]");
		return false;
	}
	target = number;
	return true;
...
void parseRegion(const JsonNode & region, ScenarioStep & target, const std::string & where, std::vector<std::string> & errors)
{
	if(!region.isStruct())
	{
...
void parseStepFields(const JsonNode & node, ScenarioStep & step, const std::string & where, std::vector<std::string> & errors)
{
	switch(step.kind)
	{
	...
	case ScenarioStep::Kind::WAIT_UNTIL:
		...
		if(node["equals"].getType() != JsonNode::JsonType::DATA_NULL && node["changes"].getType() != JsonNode::JsonType::DATA_NULL)
		{
...
bool parseStepList(const JsonNode & node, const std::string & field, bool prelude, std::vector<ScenarioStep> & target, std::vector<std::string> & errors)
{
...
ScenarioParseResult parseScenarioText(const std::string & text)
{
	...
	const std::set<std::string> allowedTopLevel = {"schema", "id", "profile", "fixture", "timeout_ms", "prelude", "steps"};
		...
		for(const auto & step : list)
		{
			if(step.kind == ScenarioStep::Kind::ATTACH)
			{
				if(!createdDevices.insert(step.device).second)
					result.errors.push_back(section + "[" + std::to_string(step.index) + "]: device '" + step.device + "' attached twice");
				continue;
			}
			...
			if(step.kind == ScenarioStep::Kind::DETACH)
			{
				...
				if(!createdDevices.count(step.device))
					result.errors.push_back(section + "[" + std::to_string(step.index) + "]: references unknown device '" + step.device + "'");
				continue;
			}
			if(!createdDevices.count(step.device))
				result.errors.push_back(section + "[" + std::to_string(step.index) + "]: references unknown device '" + step.device + "'");
...
bool validateScenarioDevices(const ScenarioSpec & scenario, const std::vector<std::string> & knownProfileIds, std::vector<std::string> & errors)
{
	bool valid = true;
	auto checkSteps = [&](const std::vector<ScenarioStep> & list, const std::string & section)
	{
		for(const auto & step : list)
		{
			if(step.kind != ScenarioStep::Kind::ATTACH)
				continue;
			if(std::find(knownProfileIds.begin(), knownProfileIds.end(), step.profileId) == knownProfileIds.end())
			{
