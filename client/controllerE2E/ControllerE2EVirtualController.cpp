...
struct PrimedButtonFilter
{
	SDL_JoystickID instanceId = -1;
	std::set<Uint8> buttons;
};

int keepNonPrimedButtonDown(void * userdata, SDL_Event * event)
{
	const auto & filter = *static_cast<const PrimedButtonFilter *>(userdata);
	return event->type != SDL_CONTROLLERBUTTONDOWN
		|| event->cbutton.which != filter.instanceId
		|| !filter.buttons.count(event->cbutton.button);
}
...
)
{
	...
	{
		errors.push_back(
			context + " must be an integer in [" + std::to_string(minValue) + ", " + std::to_string(maxValue) + "]"
		);
		return false;
	}
...
std::optional<VirtualControllerProfile>
VirtualControllerProfile::parse(const JsonNode & node, std::vector<std::string> & errors)
{
...
bool SDLVirtualController::primeButtonsPressed(const std::vector<std::string> & controls, std::string & error)
{
	if(controls.empty())
		return true;

	std::vector<std::string> resolvedControls;
	std::set<Uint8> filteredButtons;
	for(const auto & control : controls)
	{
		const auto rawButton = profile.buttonIndex(control);
		if(!rawButton)
		{
			error = "device '" + alias + "' has no button control '" + control + "'";
			return false;
		...
		std::optional<Uint8> controllerButton;
		for(int button = 0; button < SDL_CONTROLLER_BUTTON_MAX; ++button)
		{
			const auto binding = SDL_GameControllerGetBindForButton(
				controllerHandle, static_cast<SDL_GameControllerButton>(button));
			if(binding.bindType == SDL_CONTROLLER_BINDTYPE_BUTTON && binding.value.button == *rawButton)
			{
				controllerButton = static_cast<Uint8>(button);
				break;
			}
		}
		if(!controllerButton)
		{
			error = "device '" + alias + "' button control '" + control + "' has no GameController binding";
			return false;
		}
		resolvedControls.push_back(control);
		filteredButtons.insert(*controllerButton);
	}

	std::vector<std::string> primedControls;
	for(const auto & control : resolvedControls)
	{
		if(!setButton(control, true, error))
		{
			for(const auto & primed : primedControls)
			{
				std::string rollbackError;
				setButton(primed, false, rollbackError);
			}
			return false;
		}
		primedControls.push_back(control);
	}
	SDL_PumpEvents();
	PrimedButtonFilter filter{instanceId, std::move(filteredButtons)};
	SDL_FilterEvents(keepNonPrimedButtonDown, &filter);
	return true;
}
...
bool SDLVirtualController::setAxis(const std::string & control, double normalized, std::string & error)
{
	...
	if(profile.isTrigger(control))
	{
		...
		const double range = static_cast<double>(SDL_JOYSTICK_AXIS_MAX) - SDL_JOYSTICK_AXIS_MIN;
		value = static_cast<Sint16>(std::round(SDL_JOYSTICK_AXIS_MIN + clamped * range));
	}
	else
	{
...
bool SDLVirtualController::neutralize(std::string & error)
{
	...
	{
		const Sint16 value = profile.isTrigger(control) ? SDL_JOYSTICK_AXIS_MIN : 0;
		if(SDL_JoystickSetVirtualAxis(handle, index, value) != 0)
		{
